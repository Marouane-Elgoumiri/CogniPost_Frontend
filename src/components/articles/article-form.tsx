	...
	const onSubmit = async (data: ArticleFormData) => {
		...
		try {
			...
			if (isEdit && article) {
				...
				toast.success(isPublished ? 'Article published successfully' : 'Article updated successfully');
				router.push(`/articles/${article.slug}`);
			} else {
				const newArticle = await articleService.create(payload);
				toast.success(isPublished ? 'Article published successfully' : 'Article saved as draft');
		...
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
			<div className="flex items-center justify-between p-4 border rounded-lg bg-muted/50">
				<div className="space-y-0.5">
					<Label htmlFor="publish-switch" className="text-base font-medium">
